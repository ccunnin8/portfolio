...
const projectsData = {
    "campusExplorer": {
        ...
        desc: "Using Typescript and without the help of third-party libraries," +
            "I created a domain specific language that can be used to query data according to"  +
        "the specifications set forth in the class's EBNF. I wrote a program that would parse" +
        "zipped files that contained either CSV or XML documents that encoded information about" +
        "specific classes or buildings on the UBC campus. The DSL has the ability to perform" +
        "complex queries including chained and/or statements, can group data together, and perform" +
        "calculations such as min, max, count, sum, and average. The final sprint involved developing a" +
        "RESTful application that utilized the back end and provided endpoints to post new data to be parsed" +
        "and saved on the server, a UI to complete and view the results of queries, and delete datasets." +
        "While not required, I decided to publish my project to Heroku, the link is included here.",
...
for (const project of projects) {
    project.addEventListener("click", function() {
        ...
        this.scrollTo(0,0);
        // get the name of the project through the data attribute 
        const name = this.dataset.name;
        const { title, imgs, tech, desc, github } = projectsData[name];
        // use data to set fields for template 
        ...
        // loop through screenshot + descriptions and display them
        if ("imgs" in projectsData[name]) {
            for (const img of imgs) {
                ...
                desc.appendChild(p);
                newImg.setAttribute("alt", "picture of screenshot for " + title);

                // alter order with each screenshot
                if (count % 2 === 0) {
...
for (const b of back) {
    b.addEventListener("click", () => {
        ...
        // reset innerHTML so that projects don't keep appending to template 
